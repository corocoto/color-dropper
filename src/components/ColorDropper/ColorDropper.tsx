...
const ColorDropper = (props: ColorDropperProps) => {
  ...
  const updateGlassPosition = useCallback(
    ({ glass, updatedX, updatedY, glassRadius }: UpdateGlassPosition) => {
      glass.style.left = `${updatedX - glassRadius}px`
      glass.style.top = `${updatedY - glassRadius / 2}px`

      glass.style.backgroundPosition = `-${(updatedX * zoom - glassRadius + borderWidth) * 1.5}px -${
        (updatedY * zoom - glassRadius + borderWidth) * 1.6
      }px`
    },
    [zoom]
  )
